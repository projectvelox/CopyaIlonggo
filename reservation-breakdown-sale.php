<?php 
	...
	$result = mysqli_query($con, "SELECT FORMAT(SUM(price),2) AS totalPrice FROM user_cart WHERE status='Reserved' AND user='$name'");
	...
	<div class="container cswd-container" style="margin-top: 40px !important;">
		...
		<div class="table-responsive"> 
			<table class="table table-striped">
				...
				<?php
					$i = 0;
					$result = mysqli_query($con,"SELECT * FROM user_cart WHERE status='Reserved' AND user='$name'");
					while($row = mysqli_fetch_array($result))
					{
						$transactionDate = date('M j Y g:i A', strtotime($row['transaction_date']));
						$i++;
						echo "<tr>";
						echo "<td>" . $i . ".</td>";
