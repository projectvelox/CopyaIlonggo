...
<body>
	...
	<div class="container cswd-container" style="margin-top: 40px !important;">
		<ol class="breadcrumb">
		  ...
		  <li class="breadcrumb-item active"><span><?php echo $login_fullname; ?>'s Reserved Items</span></li>
		</ol>

		<?php
		$result = mysqli_query($con,"SELECT * FROM user_cart WHERE user='$login_fullname' AND status='Reserved'");
		...
		<div class="row text-center" style="border: 1px solid #ddd; padding: 20px 0px; margin-left: 0px; margin-right: 0px; margin-top: 0px; margin-bottom: 0px;">
			...
			<h2 id="ci-header" style="margin-bottom: 0px; margin-left: 0px; margin-right: 0px;"><span class="glyphicon glyphicon-calendar"></span> <?php echo $login_fullname ?>'s Reserved Items - For Sale</h2>
				...
				<?php
					...
					$result = mysqli_query($con,"SELECT * FROM user_cart WHERE user='$login_fullname' AND status='Reserved'");
						while($row = mysqli_fetch_array($result))
						{
							...
							echo '<td class="text-center"> Unclaimed </td>';
							echo '</tr>';
						}
						mysqli_close($con);
				?>
			...
			<h4 class='text-right' style="padding-right: 25px; margin-bottom: 0px;"><strong>Total: <?php
					$con = mysqli_connect("localhost","root","","ci");
					$result = mysqli_query($con,"SELECT FORMAT(SUM(price),2) as price FROM user_cart WHERE user='$login_fullname' AND status='Reserved'");
						while($row = mysqli_fetch_array($result))
						{
							echo '₱ ' . $row['price'];
						}
		...
		<?php 
		$result = mysqli_query($con,"SELECT DATE_FORMAT(date_borrowed, '%M %d, %Y') AS date_borrowed_1, DATE_FORMAT(date_due, '%M %d, %Y') AS date_due_1, FORMAT(total_price, 2) AS total_price,borrower_cart.* FROM borrower_cart WHERE name='$login_fullname' AND status='Reserved'");
		if(mysqli_num_rows($result)== 0){ } else { ?>

		<div class="row text-center" style="border: 1px solid #ddd; padding: 20px 0px; margin-left: 0px; margin-right: 0px; margin-top: 0px; margin-bottom: 0px;">
			<h2 id="ci-header" style="margin-bottom: 0px; margin-left: 0px; margin-right: 0px;"><span class="glyphicon glyphicon-calendar"></span> <?php echo $login_fullname ?>'s Reserved Items - For Rent</h2>
			<div class="responsive-table">
				<table class="table table-striped">
				<tr>
					<th>#</th>
			...
			<h4 class='text-right' style="padding-right: 25px; margin-bottom: 0px;"><strong>Total: <?php
					$con = mysqli_connect("localhost","root","","ci");
					$result = mysqli_query($con,"SELECT FORMAT(SUM(total_price),2) as total_price FROM borrower_cart WHERE name='$login_fullname' AND status='Reserved'");
						while($row = mysqli_fetch_array($result))
						{
		...
		<?php } ?>
		<br><p class="text-center"> Please claim your reserved items at Copya Ilonggo, also take note that upon arrival at the Copya Ilonggo you will be required to present a <strong>Valid ID</strong> for proof that you are the one who reserved the said items.</p>
	</div>
	<footer>
